...
_RUN_AWS_INTEGRATION = os.environ.get("PYTEST_STEPFUNCTIONS_RUN_AWS_INTEGRATION") == "1"

pytestmark = [
    pytest.mark.skipif(
        not _RUN_AWS_INTEGRATION,
        reason=(
            "Set PYTEST_STEPFUNCTIONS_RUN_AWS_INTEGRATION=1 to run AWS backend integration tests."
        ),
    ),
    pytest.mark.sfn(definition=WORKFLOW_PATH, name="OrderStatusAwsIntegration"),
]
...
def test_aws_backend_executes_tutorial_workflow(
    sfn_run: RunFixture,
    scenario: Scenario,
    expected_output: dict[str, Any] | None,
    expected_error: str | None,
) -> None:
    result = sfn_run(scenario, backend="aws")
